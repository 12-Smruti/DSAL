...
{
    int i, j, total = 0;
    for(i = 0; i < numv; i++) // initialize the values
    {
        t[i].pv = -1;
        t[i].ver_no = i;
        t[i].known = false;
    ...
    for(i = 0; i < numv; i++)
    {
        if(cost[source][i] != inf) // update values of adjacent nodes of source in table
        {
            t[i].pv = source;
            t[i].ver_no = i;
            t[i].known = false;
...
void prims::spanning()
{
    ...
    for(i = 0; i < numv; i++) // calculate total minimum cost of edges included in MST
        total += t[i].dv;
